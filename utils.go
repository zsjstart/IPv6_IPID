...
func SpoofingProbe(ip, dst_ip, proto string, port, dst_port uint16, domain string, num uint16, flag string){
	...
	for i := uint16(0); i < num; i++ {
		...
		case "udp":
			...
			SendRawUdpPackets(nil, localIP, extIP, localPort, port, 1)
...
func RscanIpidVelocityTestIPV2(sip, ip, proto, flag string, port uint16, domain string) (int) {
	...
	case "udp":
		...
		riph,  _ = RscanRecvByRawConn(nil, extIP, localIP, port, localPort, txid, "icmp", "", "udp", 0, nil, timeout) // receive ICMP unreachable packets
...
func SendRawUdpPackets(rawConn *ipv4.RawConn, srcIP, dstIP net.IP, srcPort, dstPort uint16, rn int) int {
	...
	for {
		...
		if l := packet.Layer(layers.LayerTypeICMPv4); l != nil {
			...
			if strings.Contains(needle, "udp") && uint16(ricmpl.TypeCode) == 771 { // icmp port unreachable message
				//fmt.Printf("Received ICMP port unreachable message: %v:%v\n", packet, riph.ID)
				return riph, packet
			}
			if uint16(ricmpl.TypeCode) == 0 { // icmp reply
